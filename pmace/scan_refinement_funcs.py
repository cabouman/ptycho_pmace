...
def forward_propagation(cmplx_img, cmplx_probe, patch_crd, curr_meas, x_ofs=0, y_ofs=0):
    ...
    Args:
        cmplx_img (array): Full-sized complex transmittance image.
        cmplx_probe (array): Complex-valued probe function.
        patch_crd (tuple): Coordinates that determine the current scan location.
        curr_meas (array): Current measurement associated with the patch_crd.
...
def shift_position(cmplx_img, patch_crds, offset=[0, 0]):
    ...
    Args:
        ...
        offset (list, optional): A list specifying the horizontal and vertical shift to apply to the current patch position.
            Defaults to [0, 0].
    ...
    crd0, crd1, crd2, crd3 = patch_crds # [0], patch_crds[1], patch_crds[2], patch_crds[3]
    
    # Calculate patch height and width
    # patch_h, patch_w = patch_crds[1] - patch_crds[0], patch_crds[3] - patch_crds[2]
...
def compare_result_with_ground_truth_img(cmplx_img, ref_img, display_win=None, save_dir=None,
    ...
    """ Compare reconstructed image with ground truth image and plot the error images. """
...
def compare_scan_loc(curr_scan_loc, ground_truth_scan_loc, save_dir=None):
    """ Compare current scan location with ground truth scan location and plot histgram of offsets. """
    # Initialization
...
def plot_heap_map(cmplx_img, cmplx_probe, patch_crd, curr_meas, step_sz, save_dir=None, true_offset=[0, 0]):
    """
    ...
    """
    partial_function = partial(forward_propagation, cmplx_img, cmplx_probe, patch_crd, curr_meas)
    v_offset = [-4, -3, -2, -1, 0, 1, 2, 3, 4]
    # v_offset = [-3, -2, -1, 0, 1, 2, 3]
    h_offset = [-4, -3, -2, -1, 0, 1, 2, 3, 4]
    ...
    # print(H.shape, V.shape, fvmx.shape, np.transpose(fvmx).shape)
    ...
    # Highlight minimum value by drawing a rectangle around it
    ...
    # Highlight a specific location (e.g., (4, 4)) by drawing a rectangle around it
