...
def build_parser():
    ...
    parser.add_argument('config_dir', type=str, help='Path to config file.', nargs='?',
                        const='config/recon_experiment.yaml',
                        default=os.path.join(root_dir,
                                             'experiment/synthetic_data_experiment/config/recon_experiment.yaml'))
    return parser
...
def main():
    ...
    pmace_instance = pmace2.PMACE(diffraction_data, projection_coords, init_obj, ref_obj=obj_ref, ref_probe=probe_ref,
                                  probe_exp=probe_exp, recon_win=display_win, save_dir=pmace_dir)
    pmace_result2 = pmace_instance.recon(num_iter=num_iter, rho=rho, use_reg=False, joint_recon=joint_recon,
                                         beta_patch=1 - alpha)

    # pmace_result = pmace.pmace_recon(diffraction_data, projection_coords, init_obj, obj_ref=obj_ref,
    #                                  probe_ref=probe_ref,
    ...
    # reg-PMACE recon
    cur_obj = pmace_result2['obj_revy']
    ...
    reg_instance = pmace2.PMACE(diffraction_data, projection_coords, cur_obj, ref_obj=obj_ref, ref_probe=probe_ref,
                                probe_exp=probe_exp, recon_win=display_win, save_dir=pmace_dir)
    reg_result2 = reg_instance.recon(num_iter=num_iter, rho=rho, use_reg=True, joint_recon=joint_recon,
                                         beta_patch=1 - alpha)
    ...
    nrmse = {  # 'ePIE': epie_result['obj_err'], 'WF': wf_result['obj_err'], 'AWF': awf_result['obj_err'],
        ...
        'PMACE': pmace_result2['obj_err']}  # , 'reg-PMACE': reg_pmace_result['obj_err']}
