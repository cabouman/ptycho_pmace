...
def wf_recon(init_obj_guess, diffract_data, projection_coords, obj_ref, probe_ref,
             num_iter=100, param=1, accel=True, cstr_win=None, save_dir=None):
    """
    ...
    for i in range(num_iter):
        ...
        obj_est = wf_obj_func(current_est, probe, diffract_data, projection_coords, img_norm, param=param)
        ...
        diffraction_est = np.abs(compute_ft(probe*img2patch(np.copy(obj_est), projection_coords, diffract_data.shape)))
        diffr_nrmse_val = compute_nrmse(diffraction_est, diffract_data, np.ones(diffract_data.shape))
...
def wf_joint_recon(init_obj, init_probe, diffraction_data, projection_coords, obj_ref=None, probe_ref=None,
                   num_iter=100, obj_pm=1, probe_pm=1, accel=False, cstr_win=None, save_dir=None):
    """
    ...
    start_time = time.time()
    # WF reconstruction
    print('{} joint recon starts ...'.format(approach))
    for i in range(num_iter):
        if accel:
            beta = (i + 2) / (i + 4)
        else:
